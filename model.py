...
def STEmbedding(SE, TE, T, D, bn, bn_decay, is_training):
    ...
    SE = FC(
        SE, units = [D, D], activations = [tf.nn.relu, tf.nn.relu],
        bn = bn, bn_decay = bn_decay, is_training = is_training)
    # temporal embedding
    ...
    TE = FC(
        TE, units = [D, D], activations = [tf.nn.relu, tf.nn.relu],
        bn = bn, bn_decay = bn_decay, is_training = is_training)    
    return tf.add(SE, TE)
...
def spatialAttention(X, STE, K, d, bn, bn_decay, is_training):
    ...
    X = FC(
        X, units = [D, D], activations = [tf.nn.relu, tf.nn.relu],
        bn = bn, bn_decay = bn_decay, is_training = is_training)
    return X
...
def gatedFusion(HS, HT, D, bn, bn_decay, is_training):
    ...
    H = FC(
        H, units = [D, D], activations = [tf.nn.relu, tf.nn.relu],
        bn = bn, bn_decay = bn_decay, is_training = is_training)
    return H
